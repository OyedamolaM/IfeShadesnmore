...
function AdminOverlay({
  ...
  const openReviewEditor = (index = -1) => {
  ...
  const openFaqEditor = (index = -1) => {
    const items = listForSetting("faqItems");
    const item = index >= 0
      ? items[index]
      : { question: "", answer: "" };
    setSettingsEditor({ kind: "faqItem", index, label: index >= 0 ? "Edit FAQ" : "Add FAQ" });
    setSettingsModalDraft({ ...item });
    openModal("settings");
  };
  ...
  const handleSettingsEditorSubmit = async (event) => {
    ...
    if (settingsEditor.kind === "reviewItem") {
      const currentItems = listForSetting("reviewItems");
      const nextItem = {
        name: String(settingsModalDraft.name || "").trim(),
      ...
      if (!nextItem.name || !nextItem.text) {
        pushAdminToast("Review name and text are required.", "error");
        return;
      }
      nextSettings.reviewItems = settingsEditor.index >= 0
        ? currentItems.map((item, index) => (index === settingsEditor.index ? nextItem : item))
        : [...currentItems, nextItem];
    }

    if (settingsEditor.kind === "faqItem") {
      const currentItems = listForSetting("faqItems");
      const nextItem = {
        question: String(settingsModalDraft.question || "").trim(),
        answer: String(settingsModalDraft.answer || "").trim()
      };
      if (!nextItem.question || !nextItem.answer) {
        pushAdminToast("FAQ question and answer are required.", "error");
        return;
      }
      nextSettings.faqItems = settingsEditor.index >= 0
        ? currentItems.map((item, index) => (index === settingsEditor.index ? nextItem : item))
        : [...currentItems, nextItem];
    }
    ...
    <div className={`la-admin la-theme-${adminTheme} ${isNavOpen ? "is-nav-open" : ""}`}>
              ...
              <div className="la-settings-panels">
                ...
                {settingsSection === "homepage" ? (
                  <>
                    ...
                    <section className="la-card la-settings-card">
                      <h2>Reviews</h2><p>Edit customer reviews shown after the blog section.</p>
                      <div className="la-benefit-list">
                        {listForSetting("reviewItems").map((item, index) => (
                          <div key={`review-item-${index}`} className="la-shipping-tier-row">
                            <span><strong>{item.name || "Untitled review"}</strong><small>{item.role || "Customer"} · {Number(item.rating) || 5}/5 · {item.text || "-"}</small></span>
                            <div className="la-inline-actions">
                              <button type="button" onClick={() => openReviewEditor(index)}>Edit</button>
                              <button type="button" className="la-danger-text" onClick={(event) => removeSettingsListItemAndSave(event, "reviewItems", index)}>Remove</button>
                            </div>
                          </div>
                        ))}
                      </div>
                      <button type="button" className="la-primary-button" onClick={() => openReviewEditor()}>Add review</button>
                    </section>
                    <section className="la-card la-settings-card">
                      <h2>FAQ</h2><p>Edit common questions shown below reviews.</p>
                      <div className="la-benefit-list">
                        {listForSetting("faqItems").map((item, index) => (
                          <div key={`faq-item-${index}`} className="la-shipping-tier-row">
                            <span><strong>{item.question || "Untitled question"}</strong><small>{item.answer || "-"}</small></span>
                            <div className="la-inline-actions">
                              <button type="button" onClick={() => openFaqEditor(index)}>Edit</button>
                              <button type="button" className="la-danger-text" onClick={(event) => removeSettingsListItemAndSave(event, "faqItems", index)}>Remove</button>
                            </div>
                          </div>
                        ))}
                      </div>
                      <button type="button" className="la-primary-button" onClick={() => openFaqEditor()}>Add FAQ</button>
                    </section>
                  </>
                ) : null}
      ...
      {activeModal === "settings" && settingsEditor ? (
        <form className="la-modal-card is-open" onSubmit={handleSettingsEditorSubmit}>
          ...
          {settingsEditor.kind === "reviewItem" ? (
            <>
              <label>Name<input value={settingsModalDraft.name || ""} onChange={(event) => setSettingsModalDraft((current) => ({ ...current, name: event.target.value }))} required /></label>
          ...
          {settingsEditor.kind === "faqItem" ? (
            <>
              <label>Question<input value={settingsModalDraft.question || ""} onChange={(event) => setSettingsModalDraft((current) => ({ ...current, question: event.target.value }))} required /></label>
              <label>Answer<textarea rows={4} value={settingsModalDraft.answer || ""} onChange={(event) => setSettingsModalDraft((current) => ({ ...current, answer: event.target.value }))} required /></label>
            </>
          ) : null}
